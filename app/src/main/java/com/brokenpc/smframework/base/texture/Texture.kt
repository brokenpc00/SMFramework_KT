...
abstract class Texture(director: IDirector, key: String, loadAsync: Boolean, listener: OnTextureAsyncLoadListener?) {
    ...
    inner class TextureLoaderTask : SMAsyncTask<Void?, Void?, Bitmap?> {
        private var _weakReference:WeakReference<OnTextureAsyncLoadListener>? = null
        private var _cancelled:Boolean = false

        constructor(director: IDirector, listener: OnTextureAsyncLoadListener?) : super (director) {
            _weakReference = WeakReference(listener)
        }
