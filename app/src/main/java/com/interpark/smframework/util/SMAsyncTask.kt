...
abstract class SMAsyncTask<Params, Progress, Result>(director: IDirector) : Ref(director) {
    ...
    private var _worker:WorkerRunnable<Params, Result>? = null
    private var _future:FutureTask<Result>? = null
    ...
    companion object {
        ...
        private abstract class WorkerRunnable<Params, Result> : Callable<Result> {
            lateinit var _params: Array<Params>
        }
    ...
    protected open fun publishProgress(vararg values: Progress) {
        if (!isCancelled()) {
            _director!!.getScheduler().performFunctionInMainThread(object : PERFORM_SEL {
                override fun performSelector() {
                    onProgressUpdate(values as Progress)
                }
            })
        }
    }
    ...
    init {
        _worker = object : WorkerRunnable<Params, Result>() {
            ...
            override fun call():Result? {
                ...
                return postResult(doInBackground(_params as Params))
            }
        }
